...
from get_data import point_data, processing
import time
import numpy as np
...
model_name = 'modelD virtual dataset'
...
train_dataset = point_data(NUM_POINT, NUM_POINT_SCENE, DATASET, train=True)
test_dataset = point_data(NUM_POINT, NUM_POINT_SCENE, train=False)
train_dataloader = DataLoader(train_dataset, batch_size=BATCH_SIZE, shuffle=True, num_workers=0)
test_dataloader = DataLoader(test_dataset, batch_size=BATCH_SIZE, shuffle=True, num_workers=0)

seg = SADGCNN(NUM_CLASS, k1=16, k2=4, bn=True).to(device)
if DATASET != 'virtual':
    seg.load_state_dict(torch.load('trained/'+model_name[:7]+'virtual dataset.pt'))
...
def train(model, device, train_dataloader, optimizer):
    ...
    union = 0.
    start = time.time()
    for i, (point, label, scene) in enumerate(train_dataloader):
        point, label, scene = point.to(device).float(), label.to(device).long(), scene.to(device).float()
        point = processing(point)
        scene = processing(scene)
        pred = model(point, scene)
...
def test(model, device, test_dataloader):
    ...
    start = time.time()
    with torch.no_grad():
        for i, (point, label, scene) in enumerate(test_dataloader):
            point, label, scene = point.to(device).float(), label.to(device).long(), scene.to(device).float()
            pred = model(point, scene)
            loss = loss_fn(pred, label)
